...
//    }
  ...
  addEvents: function () {
    ...
    $('.close_btn2').unbind().bind('click', function (e) {
      e.preventDefault();
      $('#room_list_tr').hide();
      $('#game_scoreboard_tr').show();
      clearInterval(self.roomTimer);
    });

    $('.close_btn1').unbind().bind('click', function (e) {
      e.preventDefault();
      $('#bet_history_tr').hide();
      $('#video_tr').show();
    });
    ...
    $('.go_lobby').unbind('click').bind('click', function () {
      if (self.dealFlag) {
        Helper.confirm.ok(WARNING, CANNOT_LEAVE_ROOM);
      } else {
...
//      }
    ...
    $('.btn_lobby').unbind('click').bind('click', function (e) {
      e.preventDefault();
      if (self.dealFlag) {
        Helper.confirm.ok(WARNING, CANNOT_LEAVE_ROOM);
      } else {
  ...
  api: {
    ...
    post: {
      ...
      updateLastConfirm: function () {
        ...
        $('#winning-amount').find('div').html(0);
      ...
      gameWinningResultRealTime: function (resp) {
        ...
        if (resp.data.status == 'WIN' || resp.data.status == 'TIE' || resp.data.status == 'VOID') {
          ...
          if (resp.data.status == 'WIN') {
            ...
            $('#winning-amount').find('div').html(`${Helper.format.number.withComma(resp.data.winning_amount)}`);
...
$(function () {
  ...
  //   const ori_width = 1500;
  //   const ori_height = 800;
  ...
  //     $('.container').css('width', `${ori_width}px`);
  //     $('.container').css('zoom', scale_val);
  //     $('.container').css('height', `${win_height - 50}px`);
  //   } else {
