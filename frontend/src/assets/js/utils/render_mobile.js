const RenderUtil = {
  ...
  renderCashOutList(data, paginate) {
    ...
    popUpData = `<table class="info_list" style="width: 1310px; margin-left: -5.5%;">
      <tbody>
      <tr>
      <th>${NICKNAME}</th>
      <th>${AMOUNT}</th>
      ...
      <nav aria-label="Page navigation" id="table-pagination" class="m-0">
      </nav>
      </div>
      </div>`;
    ...
    var elemHtml = `<div>
      ...
      <div class="cs" style="width: 1180px;">
      <div class="m_tab notice_list" style="transform: scale(0.9);">
      ${popUpData}
      </div>
      </div>
      <div class="sub_btn cashout_request">${CASHOUT}</div>
      </div>`;
  ...
  renderCashInList(data, paginate) {
    ...
    var elemHtml = `<div>
      ...
      <div class="sub_btn cashin_request">${DEPOSIT_REQUEST}</div>
      </div>`;
  ...
  renderBetHistory(data, paginate) {
    ...
    popUpData = `<table class="info_list" style="width: 1310px!important;">
      <tbody>
      <tr>
      <th width="100">#</th>
      ...
      <th width="110">winning ${AMOUNT} </th>
      <th width="110">${REMAINING_POINTS}</th>
